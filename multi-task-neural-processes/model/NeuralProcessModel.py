...
class NeuralProcessEncoder(BaseModel):
    """
    Implements Neural Process for functions of arbitrary dimensions.
    ...
    """
    def __init__(self, x_dim, y_dim, r_dim, z_dim, h_dim):
        super(NeuralProcessEncoder, self).__init__()
        self.x_dim = x_dim  # e.g., 2048
        self.y_dim = y_dim  # e.g., 1
        self.r_dim = r_dim  # e.g., 50
        self.z_dim = z_dim  # e.g., 50
        self.h_dim = h_dim  # e.g., 50
        ...
        #     Decoders[i] = Decoder(x_dim, z_dim, h_dim, y_dim)
        # self.xz_to_y = Decoders
    ...
    def forward(self, x_context, y_context, x_target, y_target=None):
        ...
        if self.training:
            ...
            # return p_y_pred, q_target, q_context
            return z_sample, q_target, q_context
        else:
            # At testing time, encode only context
            mu_context, sigma_context = self.xy_to_mu_sigma(x_context, y_context)
            # Sample from distribution based on context
...
class NeuralProcessDecoder(BaseModel):
    def __init__(self, x_dim, y_dim, z_dim, h_dim):
        super(NeuralProcessDecoder, self).__init__()
        self.x_dim = x_dim
        self.y_dim = y_dim
        self.z_dim = z_dim
        self.h_dim = h_dim
    ...
    def forward(self, x_context, y_context, x_target, z_sample):
        """
        Given context pairs (x_context, y_context) and target points x_target,
        returns a distribution over target points y_target.
